...
$$ \| x \in y \| = \bigvee_{z \in \mathcal{D}(y)} \| x = z \| . y(z) $$  
$$ \| x \subset y \| = \bigwedge_{z \in \mathcal{D}(x)} \| z \in x \| \Rightarrow y(z) $$  
...
When studying the model theory of set theory, legitimate models are the primary objects of study. The goal of the technique of forcing is the construction of new legitimate models of ZFC with interesting properties. Let's see how we can use $V^B$
