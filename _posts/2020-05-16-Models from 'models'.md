...
Let's recall the precise inductive definition of M-interpretation for an arbitrary first-order language $L$, with function constants $\Omega$ and relation constants $\Pi$. A model $M$ is a set with a function and relation for each member of $\Omega$ and $\Pi$ respectively. 
