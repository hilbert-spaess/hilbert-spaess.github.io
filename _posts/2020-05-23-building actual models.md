...
If $$\| x = y \| \in U$$, then we want $x=y$ to be true in the quotient model. Thus the objects of the model $V^{\mathbb{B}}/U$ should be equivalence classes $[x]$ of names of $V^{\mathbb{B}}$:

$$[x] \equiv [y] \Leftrightarrow \| x = y \| \in U$$
$$[x] \in [y] \Leftrightarrow \| x \in y \| \in U$$

It is easily checked that the relation $\in$ is well-defined on equivalence classes. Thus $
